var util = {
	options: {
		ACTIVE_COLOR: "#007aff",
		NORMAL_COLOR: "#000",
		subpages: ["index.html","setting.html"]
	},
	/**
	 *  简单封装了绘制原生view控件的方法
...
//		currIndex = currIndex * 2;
		...
		util.updateSubNView(currIndex, true);
//		util.updateSubNView(currIndex + 1, util.options.ACTIVE_COLOR);
		...
		for(var i = 0; i < nviewObj.tags.length-1; i++) {
			if(i !== currIndex) {
	 ...
	 */
	updateSubNView: function(currIndex, flag) {
		var self = plus.webview.currentWebview(),
		nviewEvent = plus.nativeObj.View.getViewById("tabBar"), // 获取nview控件对象
		nviewObj = self.getStyle().subNViews[0], // 获取nview对象的属性
		...
		var url=currTag.src;
		if(flag){
			url=currTag.src_active;
		}
		nviewEvent.drawBitmap(url, currTag.src_position,currTag.position, currTag.id);
	}
};